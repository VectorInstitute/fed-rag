...
def test_persist(text_nodes: list[KnowledgeNode]) -> None:
    ...
    assert os.path.exists(f"data/{knowledge_store.id}.parquet")

    # cleanup
    os.remove(f"data/{knowledge_store.id}.parquet")
...
def test_load(text_nodes: list[KnowledgeNode]) -> None:
    ...
    loaded_knowledge_store = InMemoryKnowledgeStore.load(knowledge_store.id)
